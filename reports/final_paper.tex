...
\end{enumerate}
The target variable for this model, $\text{Refugees}$, is a count and is, as expected, highly skewed, with many zero or near-zero values. To create a more normal and stable variance for the regression model, we applied a logarithmic transformation: $Y = \log(1 + \text{Refugees})$. The addition of $1$ ensures that zero refugee counts can be handled without resulting in undefined logarithms. The final engineered dataset consists of the features $(F_{\text{Origin}}, F_{\text{Asylum}})$ and the target variable $Y$.

\subsection{Model Justification}
We selected a \textbf{Random Forest Regressor (RFR)} as our predictive model. [Image of a Decision Tree structure]. This choice is justified by a few advantages over traditional linear models, especially for our data:
\begin{enumerate}
    \item \textbf{Non-linearity and Interactions:} The relationship between political freedom and migration is complex; it is unlikely to be a simple linear function. The RFR, being based on decision trees, can inherently capture non-linear relationships and intricate interaction effects between the origin and asylum country scores without requiring manual feature engineering.
    \item \textbf{Robustness to Data Characteristics:} RFR is robust against outliers and requires minimal data preprocessing, specifically avoiding the need for feature scaling, which is beneficial when combining features with different inherent scales (like the aggregated freedom scores that come from very different countries/circumstances).
...
The trained Random Forest Regressor will be evaluated on the held-out test set using both statistical and domain-specific metrics.
